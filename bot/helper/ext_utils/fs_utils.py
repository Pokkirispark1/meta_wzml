...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
    ...
    subtitle_file ="bot/util/sub.srt"
    

								 cmd = [
								    bot_cache['pkgs'][2], '-hide_banner', '-ignore_unknown',
								    '-i', media_file, '-i', subtitle_file,  # Include input media & external subtitle (.srt)
								    '-map', '0:v', '-map', '0:a', '-map', '0:s?', '-map', '1:0',  # Keep all existing subtitles (if any) & add new one
								    '-c:v', 'copy', '-c:a', 'copy',  # Copy video & audio without re-encoding
								    '-metadata', f'title={metadata}',
								    '-metadata:s:v:0', f'title={metadata}',
								    '-metadata:s:a:0', f'title={metadata}',
								    '-y', outfile
								]
								
								# Function to check if the input file has existing subtitles (more robust)
								 def has_subtitles(media_file):
								    result = subprocess.run(
								        [bot_cache['pkgs'][2], '-hide_banner', '-i', media_file],
								        stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True
								    )
								    return "Subtitle:" in result.stderr  # More reliable check
								
								# Store the result to avoid redundant calls
								 subtitle_exists = has_subtitles(media_file)
								
								# Preserve all existing subtitles if they exist
								 if subtitle_exists:
								    cmd.extend(['-c:s', 'copy'])
								
								# Determine the correct stream index for the new subtitle:
								# If there are existing subtitles, the new one is stream index 1; otherwise, it's index 0.
								 new_sub_index = 1 if subtitle_exists else 0
								
								# Convert the new subtitle to SRT (since it's always .srt)
								 cmd.extend([f'-c:s:{new_sub_index}', 'srt'])
								
								# Set metadata for subtitles:
								# - For existing subtitles (if they exist)
								 if subtitle_exists:
								    cmd.extend([f'-metadata:s:s:0', f'title={metadata}'])
								# - For the new subtitle
							 cmd.extend([f'-metadata:s:s:{new_sub_index}', f'title={metadata}'])
								
								# Set the new subtitle as default & forced using the correct stream index
									cmd.extend([f'-disposition:s:{new_sub_index}', 'default+forced'])
