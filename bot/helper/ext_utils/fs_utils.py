...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
    ...
    # Function to check if the input file has existing subtitles (better method using ffprobe)
    def has_subtitles(media_file):
        result = subprocess.run(
            [bot_cache['pkgs'][2], '-hide_banner', '-show_streams', '-select_streams', 's', '-print_format', 'json', '-i', media_file],
            stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True
        )
        return '"codec_type": "subtitle"' in result.stdout  # Accurate check for subtitle streams
    ...
    cmd = [
        ...
        '-map', '0:v', '-map', '0:a', '-map', '0:s?', '-map', '1:0',  # Keep all existing subtitles & add new one
    ...
    # Ensure at least one subtitle track exists before setting `-disposition`
    if subtitle_exists or new_sub_index == 0:
