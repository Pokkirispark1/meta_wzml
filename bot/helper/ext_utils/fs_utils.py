...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
    # Create a temporary SRT file with the intro text
    srt_file = os.path.join(base_dir, 'temp_subtitle.srt')
    srt_content = f"""1
...
"""
    ...
    # Count existing subtitle streams using ffprobe
    ffprobe_cmd = [bot_cache['pkgs'][1], '-v', 'error', '-select_streams', 's', 
                   '-show_entries', 'stream=index', '-of', 'csv=p=0', media_file]
    proc = await create_subprocess_exec(*ffprobe_cmd, stdout=PIPE, stderr=PIPE)
    stdout, _ = await proc.communicate()
    num_subtitles = len(stdout.decode().splitlines())  # Number of existing subtitle streams

    # Construct FFmpeg command with original settings plus new subtitle
    cmd = [bot_cache['pkgs'][2], '-hide_banner', '-ignore_unknown', 
           '-i', media_file,                # Original input file
           '-i', srt_file,                  # New subtitle input
           '-metadata', f'title={metadata}', # Original global metadata
           '-metadata:s:v', f'title={metadata}', # Original video metadata
           '-metadata:s:a', f'title={metadata}', # Original audio metadata
           '-metadata:s:s', f'title={metadata}', # Original subtitle metadata
           '-map', '0:v:0?',                # Original video mapping
           '-map', '0:a:?',                 # Original audio mapping
           '-map', '0:s:?',                 # Original subtitle mapping
           '-map', '1:0',                   # New subtitle mapping
           '-c:v', 'copy',                  # Original video codec
           '-c:a', 'copy',                  # Original audio codec
           '-c:s', 'copy',                  # Original subtitle codec (works for SRT too)
           f'-disposition:s:{num_subtitles}', 'default+forced', # Set new subtitle as default+forced
           outfile, '-y']                   # Original output settings
