...
async def modify_srt(temp_srt_path: str, new_sub_path: str, intro_text: str):
    """Modify an SRT file by adding an intro subtitle and adjusting indices."""
    # Intro subtitle: 0 to 5 seconds
    ...
    # Increment indices in original blocks
    for block in blocks:
        try:
            index = int(block[0].strip())
            block[0] = f'{index + 1}\n'
        except ValueError:
            pass  # Skip if first line isn't a number
    
    # Write new SRT: intro + modified original subtitles
    with open(new_sub_path, 'w', encoding='utf-8') as f:
        f.writelines(intro_block)
        for block in blocks:
            f.writelines(block)
...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = '', intro_text: str = 'JOIN @MOVIEMANIA_TG FOR MORE'):
    ...
    subtitle_indices = stdout.decode().strip().split('\n')
    num_subtitles = len(subtitle_indices) if subtitle_indices[0] else 0
    
    # Create temporary files
    with tempfile.NamedTemporaryFile(suffix='.srt', delete=False) as temp_srt:
        temp_srt_path = temp_srt.name
    with tempfile.NamedTemporaryFile(suffix='.srt', delete=False) as new_sub:
        new_sub_path = new_sub.name
    
    try:
        if num_subtitles > 0:
            # Extract first subtitle stream
            cmd_extract = [bot_cache['pkgs'][2], '-i', media_file, '-map', '0:s:0', '-c:s', 'srt', temp_srt_path]
            ...
            if code != 0:
                ...
                return
            # Modify subtitle with intro text
            await modify_srt(temp_srt_path, new_sub_path, intro_text)
        else:
            # Create new subtitle with intro text
            with open(new_sub_path, 'w', encoding='utf-8') as f:
                f.write(f'1\n00:00:00,000 --> 00:00:05,000\n{intro_text}\n\n')
        
        # Construct FFmpeg command
        cmd = [bot_cache['pkgs'][2], '-hide_banner', '-i', media_file, '-i', new_sub_path, 
               '-map', '0:v:0?', '-map', '0:a:?', '-map', '1:0']
        for i in range(1, num_subtitles):
