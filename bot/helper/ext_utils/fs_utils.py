...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
    ...
    '-i', media_file, '-i', subtitle_file,  # Include the SRT subtitle file
    '-map', '0:v:0', '-map', '0:a:0', '-map', '1:0',  # Ensure correct mapping
    '-c:v', 'copy', '-c:a', 'copy', '-c:s', 'srt',  # Use SRT subtitle codec
    '-metadata', f'title={metadata}',
    '-metadata:s:v:0', f'title={metadata}',
    '-metadata:s:a:0', f'title={metadata}',
    '-metadata:s:s:0', f'title={metadata}',
    '-disposition:s:0', 'default+forced',  # Mark subtitles as default & forced
    outfile, '-y'
]
    listener.suproc = await create_subprocess_exec(*cmd, stderr=PIPE)
    code = await listener.suproc.wait()
